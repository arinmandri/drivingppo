...
def gen_1t():    return generate_random_world_plain(map_h= 60, map_w= 60, num=1,  init_dist=DIS_SCFAC, min_dist=NEAR*2, max_dist=DIS_SCFAC, ang_lim=pi*1.0, ang_init='half', spd_init=0.0, near=1.5)
def gen_2t():    return generate_random_world_plain(map_h=150, map_w=150, num=2,  init_dist=DIS_SCFAC, min_dist=NEAR*2, max_dist=DIS_SCFAC, ang_lim=pi*1.0, ang_init='half', spd_init=0.0, near=NEAR-0.5)  # 학습용: 도달판정범위 약간 작게
def gen_3t(N=1): return generate_random_world_plain(map_h=300, map_w=300, num=N,  init_dist=DIS_SCFAC, min_dist=NEAR*2, max_dist=DIS_SCFAC, ang_lim=pi*1.0, ang_init='half', spd_init=0.0, near=NEAR-0.5)
